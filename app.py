...
def convertir_webm_a_wav(ruta_original):
    ...
    try:
        ...
        logging.error(f"Error al convertir audio: {e}")
        return None
...
def obtener_duracion_audio_segundos(ruta_archivo):
    try:
        ...
        logging.warning(f"No se pudo obtener duración de audio: {e}")
        return None
...
class ListPagination:
    """Clase de paginación manual para cualquier lista de Python."""
...
class MensajeProgramado(db.Model):
    ...
    mensaje=db.Column(db.Text, nullable=False) # Considerar cifrado si es información sensible
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                ...
                # Crear el objeto del mensaje para el prospecto
...
def nuevo_plan_nutricional_para_visita_page(visita_id):
    ...
    if request.method == 'GET':
        ...
        # Solo se ejecuta en la primera carga de la página.
        if visita.transcripcion and visita.transcripcion.strip() and client_openai and not visita.transcripcion.startswith("[Error"):
            datos_ia_plan = generar_contenido_plan_nutricional_con_ia(visita.transcripcion, visita.idioma_detectado or 'es')
            if datos_ia_plan and not datos_ia_plan.get("error"):
                # Se pasa el diccionario directamente a la plantilla. El JavaScript lo parseará.
             ...
             flash("No hay una transcripción válida para generar un plan nutricional automáticamente.", "warning")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            nuevo_plan_db = PlanNutricional(
                ...
                # La lógica para 'notas_adicionales' ahora se manejará mejor en el JSON.
...
def iniciar_visita():
    ...
    try:
        ...
        return jsonify({
            "mensaje": "Visita iniciada con éxito.", "visita_id": nueva_visita_db.id,
...
def api_resumir_documentos_e_iniciar_visita():
    ...
    try:
        ...
        return jsonify({
            "mensaje": final_msg, "status": res_ia_docs_status, "visita_id": n_visita_res.id,
...
def subir_audio():
    ...
    if file:
        ...
        if duracion_seg is None:
            ...
            # El mensaje flash no es muy útil en una API, pero lo mantenemos por si acaso
...
def api_actualizar_notas_resumen_ai():
    ...
    try:
        ...
        return jsonify({"mensaje": "Contenido AI actualizado exitosamente."}), 200
...
def api_crear_referencia_desde_chat():
    ...
    try:
        ...
        return jsonify({
            'mensaje': 'Documento de Referencia creado exitosamente.',
            'referencia': {
...
def transcribir_diarizar_audio():
    ...
    try:
        ...
        db.session.add(reg_act); db.session.commit()
        return jsonify({
            "mensaje": "Audio transcrito exitosamente.", "transcripcion": texto_transcrito,
...
def api_generar_resumen_ai():
    ...
    if not resumen_gen.startswith("[Error"):
        ...
        db.session.add(reg_act); db.session.commit()
        return jsonify({"mensaje": "Resumen AI generado exitosamente.", "resumen_ai": resumen_gen,
...
def api_generar_notas_ai():
    ...
    return jsonify({
        "mensaje": "Notas AI generadas exitosamente.",
...
def api_generar_plan_alimenticio():
    ...
    return jsonify({
        "mensaje": "Contenido del plan nutricional generado exitosamente por IA.",
...
def eliminar_visita_completa(visita_id_param):
    ...
    try:
        ...
        return jsonify({"mensaje": f"Visita ID {visita_id_param} (Paciente: {pac_nombre}) eliminada exitosamente."}), 200
    except Exception as e:
        db.session.rollback()
...
def compartir_visita_email():
    ...
    try:
        ...
        cuerpo_html = ""
        if image_data_b64:
            # Si se envía una imagen, el correo solo la contendrá junto a un mensaje
            ...
            return jsonify({"mensaje": f"Notas AI de la visita enviadas exitosamente como imagen a {email_dest}."}), 200
            ...
            return jsonify({"mensaje": f"Información de la visita enviada exitosamente a {email_dest}."}), 200
    except Exception as e:
